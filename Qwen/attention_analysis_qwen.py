...
def classify_tokens_final(tokens, attention_values):
    ...
    # Helper: Determine if "." is part of a floating-point number like ["3", ".", "14"]
    ...
    # Helper: Check if a token is a complete number (int, float, sci notation)
    def is_number(token):
        token = token.lower()
    ...
    for i, (tok, attn) in enumerate(zip(tokens, attention_values)):
        ...
        # Punctuation (only if not part of number)
        # Punctuation
...
def remove_leading_single_digits(token_attention_list):
    ...
    """
    token_attention_list = [t for t in token_attention_list if 72 <= t[0] <= 2544]
    cleaned = []
    i = 0
...
def extract_values_attention_by_feature_name(filtered_tokens, feature_names, k=10):
        ...
        if curr_pos == prev_pos + 1:
            ...
            if len(current_group) > 1:
                ...
                if feature_idx >= len(feature_names):
                    feature_idx = 0
                    row_idx += 1
                    if row_idx >= k:  # stop after k training rows
                        return results
            current_group = [filtered_tokens[i]]

    # Handle last group
    if len(current_group) > 1 and row_idx < k:
        total_attention = sum(attn for _, _, attn in current_group)
        feature_name = feature_names[feature_idx]
        results.append((f"{feature_name}_value", total_attention))
...
def sum_xn_values(xn_attention_list):
    ...
    return list(total_by_label.items())
...
def analyze_single_datapoint(data_idx, X_test, X_train, Y_train, features, tokenizer, model, k=10):
    try:
        ...
        # NEW: tokenization block
        prompt = tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=False)
        messages_wrapped = [{"role": "user", "content": prompt}]
        text = tokenizer.apply_chat_template(
            messages_wrapped, tokenize=False, add_generation_prompt=True, enable_thinking=False
        )
        ...
        with torch.no_grad():
            while steps < max_new_tokens:
                current_input = generated_ids if past_key_values is None else generated_ids[:, -1:]  # only new token
                outputs = model(
                ...
                attn_last_layer = outputs.attentions[-1][0]
        ...
        # Average attention over fixed input span
        input_length = len(tokenizer.convert_ids_to_tokens(input_ids[0]))
        per_step_raw = [attn.mean(dim=0)[:input_length] for attn in all_attentions]
        ave_attention_tensor = torch.stack(per_step_raw, dim=0).mean(dim=0)
        ...
        feature_result = extract_values_attention_by_feature_name(filtered, features, k=k)
...
def run_analysis_1000_datapoints():
    ...
    if not row_df.empty:
        ...
        row_df.to_csv(f"{data_name}_fewshot{k}_row_level_attention_results_qwen.csv", index=False)
        row_summary.to_csv(f"{data_name}_fewshot{k}_row_level_attention_summary_qwen.csv")
    ...
    if not name_df.empty:
        ...
        name_df.to_csv(f"{data_name}_fewshot{k}_name_level_attention_results_qwen.csv", index=False)
        name_summary.to_csv(f"{data_name}_fewshot{k}_name_level_attention_summary_qwen.csv")
    ...
    if not value_df.empty:
        ...
        value_df.to_csv(f"{data_name}_fewshot{k}_value_level_attention_results_qwen.csv", index=False)
        value_summary.to_csv(f"{data_name}_fewshot{k}_value_level_attention_summary_qwen.csv")
        
    # save generations 
    gen_df.to_csv(f"{data_name}_fewshot{k}_generated_outputs_qwen.csv", index=False)
